...
Generate a fresh, random Excel dataset (ensuring idempotency).

Send 10 distinct test scenarios to the running Docker container.

Test Happy Paths (Pivot tables, Sentiment) and Edge Cases (Missing files, 404s).

Compile the results into a professional PDF Report.

To run the test: (Ensure the Docker container is running first!)
